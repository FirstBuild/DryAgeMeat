...
void Adafruit_ILI9341::contWrite() {

  if (hwSPI) spi_begin();
  writecommand(ILI9341_RAMWRCT);
}
...
void Adafruit_ILI9341::dmaspiwrite(uint8_t* c, uint16_t length, void (*dmaCB)()) {
  // rudimentary clipping (drawChar w/big text requires this)

  SPI.transfer(c, NULL, length, dmaCB);
...
*/
  ...
  writedata(0x00);
  writedata(0x15);                  // Update frame rate to 15 from 18
...
void Adafruit_ILI9341::fillRedScreen(uint16_t color) {
    fillRedRect(0, 0, _width, _height, color);
}

void Adafruit_ILI9341::fillRedRect(int16_t x, int16_t y, int16_t w, int16_t h,
  uint16_t color) {
  ...
  uint8_t widthLine[w * 16];

  for (int i = 0; i < (w * 16); i += 2) {
    widthLine[i] = color >> 8;
    widthLine[i + 1] = color;
  }
...
for(y=h; y>0; y -= 8) {
  SPI.transfer(widthLine, NULL, w * 16, NULL);
}
